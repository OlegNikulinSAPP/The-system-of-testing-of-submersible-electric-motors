...
#include "ui_window_experience_13.h"

#include <QAxObject>
#include <QMessageBox>

window_experience_13::window_experience_13(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::window_experience_13)
{
    ui->setupUi(this);

    this->setWindowTitle("13. Ипытание изо-ции обмотки на эл. прочн. отн. корпуса");
...
window_experience_13::~window_experience_13()
{
    delete ui;
}

void window_experience_13::slotInExperienceOneNumberProtocol(QString numberProtocol)
{
    fileProtocol = numberProtocol;
}

void window_experience_13::closeEvent(QCloseEvent *)
{
    ui->checkBox_TestingIsolationBody_of_SEM_result->setCheckState(Qt::Unchecked);
}

void window_experience_13::slotFromMainTo_window_experience_13_Parameters(float TestingIsolationBody)
{
    ui->lineEdit_TestingIsolationBody_of_SEM_Nominal->setText(QString::number(TestingIsolationBody));
}

void window_experience_13::on_pushButton_TestingIsolationBody_of_SEM_save_clicked()
{
    ...
    QAxObject* data = sheet->querySubObject("Cells(int, int)", 49, 10);
    data->dynamicCall("SetValue(const QVariant&)", ui->lineEdit_TestingIsolationBody_of_SEM_Nominal->text()); // номинальное значение в протокол

    if (ui->checkBox_TestingIsolationBody_of_SEM_result->isChecked()){
        data = sheet->querySubObject("Cells(int, int)", 49, 19);
        data->dynamicCall("SetValue(const QVariant&)", "Годен");
            if (ui->checkBox_first_run_in->isChecked()){
                    data = sheet->querySubObject("Cells(int, int)", 49, 13);
                    data->dynamicCall("SetValue(const QVariant&)", "Испытание выдержал");
                } else if (ui->checkBox_second_run_in->isChecked()){
                    data = sheet->querySubObject("Cells(int, int)", 49, 16);
                    data->dynamicCall("SetValue(const QVariant&)", "Испытание выдержал");
                }
    } else {
        data = sheet->querySubObject("Cells(int, int)", 49, 19);
    ...
    close();
    emit signalExperienceThirteenIsEnded();
}
